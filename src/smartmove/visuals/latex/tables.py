def write_table(
    ...
    # Write table to file
    head = __create_header(
        headers, title, caption, adjustwidth, tiny, centering, extrarowheight
    )
...
def __create_header(
    ...
    if extrarowheight:
        if isinstance(extrarowheight, int):
            ...
            raise TypeError(
                "`extrarowheight` must be an integer value " "(font point size)"
            )
...
if __name__ == "__main__":
    ...
    head_units = ["", "(degrees)", r"(m day\textsuperscript{-1})"]
