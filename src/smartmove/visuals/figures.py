...
from pyotelem.plots import plotutils, plotglides
import scipy.optimize
import seaborn
import yamlord

from smartmove.config import paths, fnames
from smartmove.visuals import latex, utils, maps
from smartmove.ann import pre, utils_ann
...
def plot_sgls_tmbd(exps_all, path_plot=None, dpi=600):
    ...
    def r_squared(x, y):
        ...
        denr = ((n * sum(x ** 2)) - (sum(x) ** 2)) * ((n * sum(y ** 2)) - (sum(y) ** 2))
        return numr / denr
    ...
    ax1.annotate(
        table_title, xy=(0.82, 0.53), xycoords="axes fraction", fontsize=12, bbox=props
    )
    ax1.annotate(
        table_id,
        xy=(0.89, 0.05),
    ...
    ax1.annotate(
        table_rho, xy=(0.90, 0.05), xycoords="axes fraction", fontsize=10, bbox=props
    )
    ...
    ax1.set_ylabel("Sub-glides during dive descents $(\%)$")  # noqa: W605
    ...
    ax2 = ax1.twinx()
    ax2.set_ylabel("Sub-glides during dive ascents $(\%)$")  # noqa: W605
    yi_inv = 100 - yi
    ...
    if path_plot is not None:
        ...
        latex.utils.pdf_to_img(path_plot, fname, in_ext="eps", out_ext="png", dpi=dpi)
    plt.show()
...
def sgl_density(exp_name, sgls, max_depth=20, textstr="", path_plot=None):
    ...
    g.set_axis_labels(
        xlabel="Experiment duration ($min \, sec$)", ylabel="Depth ($m$)"
    )  # noqa: W605
...
def plot_sgl_histos(path_project, cfg_ann, path_plot):
    ...
    axes[0].set_yticklabels(
        [str(i) if i % 2 == 0 else "" for i in range(net_depth + 1)]
    )
...
def plot_sgl_highlight(
    path_project, cfg_ann, path_plot, idx_start=282400, idx_end=284200, clip_x=True
):
    ...
    data_paths, exp_names = utils, utils.filt_paths(path_project, cfg_ann)
...
def plot_ann_performance(cfg_ann, results_tune, path_plot):
    ...
    df_cfgs = pandas.DataFrame(index=range(len(results_tune)), columns=params + ["acc"])
...
def make_all(path_project, path_analysis, studymap=True):
    ...
    utils_ann.plot_confusion_matrix(
        cm_test,
        tick_labels,
        xlabel=xlabel,
        ylabel=ylabel,
    ...
    for fig in os.listdir(path_plot):
        if fig.endswith(".eps"):
            ...
            latex.utils.pdf_to_img(
                path_plot, fname, in_ext="eps", out_ext="png", dpi="300"
            )
