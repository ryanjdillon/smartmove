...
def table_exps(name, attrs, path_table, data):
    ...
    table = latex.tables.write_table(
        path_table,
        name,
        data,
        cols,
...
def make_all(path_project, path_analysis):
    ...
    file_cfg_ann = _join(
        path_project, paths["ann"], path_analysis, fnames["cfg"]["ann"]
    )
    ...
    table_ann_params(name_ann_params, table_attrs[name_ann_params], path_table, cfg_ann)

    # Create ANN target bin description table
    name_fdescr = table_ann_target_descr.__name__
    file_post = _join(path_project, paths["ann"], path_analysis, fnames["ann"]["post"])
    post = yamlord.read_yaml(file_post)
    data = utils.target_value_descr(post)
    table_ann_target_descr(name_fdescr, table_attrs[name_fdescr], path_table, data)
    ...
    table_ann_feature_stats(
        name_fstats, table_attrs[name_fstats], path_table, input_stats
    )

    # Create target value statistics table
    name_tstats = table_ann_target_stats.__name__
    file_train = _join(
        path_project, paths["ann"], path_analysis, fnames["ann"]["train"]
    )
    file_valid = _join(
        path_project, paths["ann"], path_analysis, fnames["ann"]["valid"]
    )
    file_test = _join(path_project, paths["ann"], path_analysis, fnames["ann"]["test"])
    ...
    table_ann_target_stats(
        name_tstats, table_attrs[name_tstats], path_table, target_stats
    )
    return None
