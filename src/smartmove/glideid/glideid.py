...
def run(
    ...
    ignore = ["nperseg", "peak_thresh", "alpha", "min_depth", "t_max", "last_modified"]
    ...
    for i in process_ind:
        ...
        file_cal_acc = _join(path_project, paths["tag"], path_cal_acc, fname_cal)
        file_cal_prop = _join(path_project, paths["csv"], fname_cal_prop)

        tag, dt_a, fs_a = utils_lleo.load_lleo(
            path_data_tag, file_cal_acc, file_cal_prop
        )
        ...
        # Find Glides
        # ------------
        GL, masks = _process_glides(
            cfg_glide_exp, tag, fs_a, dives, masks, plots=plots, debug=debug
        )
...
def _process_tag_data(
    path_project, cfg_project, cfg_glide, path_exp, tag, fs_a, plots=True, debug=False
):
    ...
    tag["dsw"] = utils_ctd.get_seawater_densities(file_ctd_mat, t, lon, lat, max_depth)
...
def _process_glides(cfg_glide, tag, fs_a, dives, masks, plots=True, debug=False):
    """
    ...
    masks["glides"] = utils.mask_from_noncontiguous_indices(
        len(tag), GL[:, 0], GL[:, 1]
    )
