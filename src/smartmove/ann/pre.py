...
def calc_mod_density(mass_kg, dens_kgm3, n_blocks, block_type):
    ...
    # Raise error if incorrect `block_type` passed
        ...
        raise KeyError(
            "`block_type` must be: {} " "- not {}".format(block_types, block_type)
        )
    return rho_mod


def simulate_density(mass_kg=40, bd_start=1000, n_bd=101, block_start=1, n_blocks=8):
    ...
    for i in range(len(df)):
        ...
        rho_mod = calc_mod_density(mass_kg, bodydensities[i], blocks[i], types[i])
...
def _compile_experiments(
    ...
    if not glide_paths_found:
        raise SystemError(
            "No glide paths found, check input directories "
            "for errors\n"
            "path_project: {}\n"
    ...
    exps_all = _add_ids_to_df(exps_all, exp_ids, animal_id=animal_ids, tag_id=tag_ids)

    return exps_all, sgls_all, dives_all


def _create_ann_inputs(
    path_project,
    cfg_ann,
    path_tag,
    path_glide,
        ...
        for exp_id in exp_ids:
            ...
            try:
                ...
                if exp_id not in numpy.unique(field["exp_id"]):
                    raise Exception(
                        "{} not in " "`field_experiments.csv`".format(exp_id)
                    )
                elif "rho_mod" not in field.columns:
                    raise Exception(
                        "`rho_mod` field not found in "
    ...
    exps_all, sgls_all, dives_all = _compile_experiments(
        path_project, path_glide, cfg_ann["data"], fname_sgls, fname_mask_sgls_filt
    )
...
def process(path_project, cfg_project, cfg_ann):
    ...
    file_field = os.path.join(path_project, paths["csv"], fnames["csv"]["field"])
    file_isotope = os.path.join(path_project, paths["csv"], fnames["csv"]["isotope"])
